...
class M extends Controller {
    ...
    public function index() {
        ...
        foreach ($this->mcat as $key => $value) {
            ...
            $dd = $model->where(['typeid'=>['in',$this->cid]])->order('pubdate desc')->cache(3600)->limit($limit)->select();
    	...
    	$huandeng = $model->limit(5)->where("find_in_set('f',flag)")->cache(3600)->field('id,title,litpic')->order('pubdate desc')->select();
    ...
    public function m_detail(){
    	...
    	$tuijian = $model->where("find_in_set('c',flag)")->cache(3600)->limit(4)->order('pubdate desc')->select();
    ...
    public function m_list(){
    	...
    	$this->sumcatid($id);
    	$cat = model('HhType')->where(['status'=>1])->cache(86400)->column('id,typename');
    	$list = $model->where(['typeid'=>['in',$this->cid]])->cache(3600)->order('pubdate desc')->paginate(12);
    ...
    public function search(){
        ...
        $model = model('HhList');
        $cat = model('HhType')->where(['status'=>1])->cache(86400)->column('id,typename');
        $list = $model->where(['title'=>['like','%'.$q.'%']])->cache(3600)->order('pubdate desc')->paginate(12,false,['query'=>request()->param()]);
        foreach ($list as $key => &$value) {
            $value['description'] = trim(mb_substr(strip_tags($value['content']), 0,50));
        }
        $page = $list->render();
    ...
    public function sumcatid($pid){
    	$cat = model('HhType')->cache(86400)->where(['status'=>1,'pid'=>$pid])->column('id');
    	if(!empty($cat)){
    ...
    function getcname($id){
        if($id != 0){
            $cat = model('HhType')->cache(86400)->where(['status'=>1,'id'=>$id])->field('pid,typename')->find();
